...
def read_csv_as_nested_dic(filename, keyfield):
	with open (filename,newline='') as csvfile:
...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	for i in plot_countries:
		list1.append(i)
	for w in gdp_countries:
		list2.append(w)
	for check in list1:
		if check in list2:
			table[check]=plot_countries[check]
		else:
			set0.add(check)
	result.append(table)
	result.append(set0)
        ...
        for row in reader:
            for code in plot_countries:
                ...
                if country==row[gdpinfo["country_name"]] and row[year]!="":
                    dic[code]=math.log10(float(row[year]))
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    mapchart.title="全球GDP分布图"
    mapchart.add(year,build_map_dict_by_name(gdpinfo,plot_countries,year)[0])
    mapchart.add("missing from world bank",build_map_dict_by_name(gdpinfo,plot_countries,year)[1])
    mapchart.add("no data at this year",build_map_dict_by_name(gdpinfo,plot_countries,year)[2])
